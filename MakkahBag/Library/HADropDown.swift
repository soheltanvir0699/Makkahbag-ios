...
//  MakkahBag
//
//  Created by appleguru on 5/6/20.
...
class HADropDown: UIView {
          ...
          rootView?.addSubview(self.table)
           ...
           self.table.reloadData()
            UIView.animate(withDuration: 0.25, animations: {
                self.table.frame = CGRect(x: self.tableFrame.origin.x, y: self.frame.origin.y + self.frame.height+5, width: self.frame.width, height: height)
    ...
    func collapseTableView() {
        ...
        UIView.animate(withDuration: 0.25, animations: {
                self.table.frame = CGRect(x: self.tableFrame.origin.x, y: self.frame.origin.y+self.frame.height, width: self.frame.width, height: 0)
            })
          var rootView = self.superview
